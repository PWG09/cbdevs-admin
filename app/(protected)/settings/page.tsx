...
export default function SettingsPage() {
  ...
  useEffect(()=>{ if(!auth.currentUser)return; setEmail(auth.currentUser.email||""); getDoc(doc(db,"users",auth.currentUser.uid)).then(s=>{if(s.exists()){const d=s.data();setName(d.name||"");setRole(d.role||"")}})},[]);
  async function save(){if(!auth.currentUser)return;await updateDoc(doc(db,"users",auth.currentUser.uid),{name});await updateProfile(auth.currentUser,{displayName:name});setSaved(true);setTimeout(()=>setSaved(false),1800)}
